...
本项目基于开源方案进行二次开发与工程化扩展，聚合了 YOLO 病虫害识别、知识图谱决策和农事任务管理等能力，构建面向智慧农业场景的“一体化数字农服平台”。通过“感知 → 诊断 → 决策 → 执行”的闭环设计，帮助种植主体实现病害早预警、精准处置和任务协同。

## 我的二次开发重点

- **病害标签扩展与映射**：补充标签解析与知识库映射逻辑，让识别结果能稳定关联到病害实体与方案库。
- **Spark LLM 集成**：新增星火大模型调用链路、聊天接口与降级策略，提升智能问答和演示完整度。
- **Prompt 与应急回复设计**：围绕病害、施药、天气、设备等场景整理默认回复策略，避免第三方接口异常时能力断层。
- **方案生成与天气联动**：将知识库、药剂信息、天气缓存和传感器数据串联，输出可执行的防治建议与施药时间窗。
- **硬件 TCP 网关与多端协同**：补充设备控制链路，增强前端、Spring Boot 中台、Flask 推理服务之间的闭环协作。

更细的改动说明见 [docs/CHANGES.md](/Users/ruyne./Library/Mobile%20Documents/com~apple~CloudDocs/ljzp/docs/CHANGES.md)。
...
- 仓库中的 `yolo_cropDisease_detection_springboot/src/main/resources/application.properties` 已改为公开安全版本。
- 你在本地运行前至少需要替换：
  - `spring.datasource.password`
  - `llm.spark.app-id`
  - `llm.spark.api-key`
  - `llm.spark.api-secret`
- 如暂时不接入星火大模型，可保留占位符，系统会自动走本地降级回复逻辑。
